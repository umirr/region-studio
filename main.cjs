...
function resultImage(s,name){const ext=path.extname(name).toLowerCase(),type=mime[ext];if(!type||typeof s!=='string'||!s.startsWith('data:'+type+';base64,'))throw Error('원본 확장자와 저장 형식이 다릅니다.');const b=Buffer.from(s.split(',')[1],'base64');const valid=ext==='.png'?b.subarray(0,8).toString('hex')==='89504e470d0a1a0a':ext==='.webp'?b.toString('ascii',0,4)==='RIFF'&&b.toString('ascii',8,12)==='WEBP':b[0]===255&&b[1]===216&&b[2]===255;if(!valid||b.length>40000000)throw Error('잘못된 이미지');return b;}
app.whenReady().then(async()=>{
 ...
 handle('write',async(token,id,p)=>{const d=destinations.get(token),item=store.item(id);if(!d)throw Error('내보내기 위치가 없습니다.');if(d.kind==='results'&&p.skipped){const state=await store.read(id);if(!state?.skipped)throw Error('스킵 상태가 아닙니다.');await fs.copyFile(item.path,path.join(d.dir,item.name));return;}if(d.kind==='results'){await atomic(path.join(d.dir,item.name),resultImage(p.image,item.name));return;}
