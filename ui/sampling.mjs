...
export function sampledCoverage(mask,w,h){
 ...
 for(let y=0;y<h;y++)for(let x=0;x<w;x++){const p=y*w+x;if(field[p]===0)continue;if(field[p]===1){out[p]=255;continue;}let coverage=0;
  const left=field[y*w+Math.max(0,x-1)],right=field[y*w+Math.min(w-1,x+1)],top=field[Math.max(0,y-1)*w+x],bottom=field[Math.min(h-1,y+1)*w+x];
  const curvature=Math.abs(left+right+top+bottom-4*field[p]);
  const samples=curvature>.12&&field[p]>.08&&field[p]<.92?16:8;
  for(let sy=0;sy<samples;sy++)for(let sx=0;sx<samples;sx++){const v=sample(x+(sx+.5)/samples-.5,y+(sy+.5)/samples-.5),t=Math.max(0,Math.min(1,(v-.2)/.6));coverage+=t*t*(3-2*t);}
  out[p]=Math.round(coverage/(samples*samples)*255);
 }return out;
}
