...
export function radialFill(input,w,h,points,edge,{strength=65,radius=35,gap=1}={}){
 const barrier=barrierMap(edge,w,h,strength,gap),out=input.slice(),r=Math.max(8,Math.min(w,h)*Math.min(radius,15)/100),directions=Array.from({length:8},(_,i)=>[Math.cos(i*Math.PI/4),Math.sin(i*Math.PI/4)]);let hits=0,filled=0;
 const negatives=seeds(points,w,h,0);
 for(const seed of seeds(points,w,h,1)){const cx=seed%w,cy=Math.floor(seed/w),ends=directions.map(([dx,dy])=>{for(let d=2;d<=r;d++){const x=Math.round(cx+dx*d),y=Math.round(cy+dy*d);if(x<0||y<0||x>=w||y>=h)return null;if(barrier[y*w+x]){hits++;return [cx+dx*(d-1),cy+dy*(d-1)];}}return null;});
 const allowed=new Uint8Array(w*h);for(let k=0;k<8;k++){const a=ends[k],b=ends[(k+1)%8];if(!a||!b)continue;const ax=a[0]-cx,ay=a[1]-cy,bx=b[0]-cx,by=b[1]-cy,det=ax*by-ay*bx;if(Math.abs(det)<.01)continue;for(let y=Math.max(0,Math.floor(Math.min(cy,a[1],b[1])));y<=Math.min(h-1,Math.ceil(Math.max(cy,a[1],b[1])));y++)for(let x=Math.max(0,Math.floor(Math.min(cx,a[0],b[0])));x<=Math.min(w-1,Math.ceil(Math.max(cx,a[0],b[0])));x++){const u=((x-cx)*by-(y-cy)*bx)/det,v=(ax*(y-cy)-ay*(x-cx))/det;if(u>=0&&v>=0&&u+v<=1)allowed[y*w+x]=1;}}
 const queue=[seed],seen=new Uint8Array(w*h);seen[seed]=1;for(let i=0;i<queue.length;i++){const p=queue[i],x=p%w,y=Math.floor(p/w);if(!allowed[p]||barrier[p]||negatives.some(n=>(x-n%w)**2+(y-Math.floor(n/w))**2<=((x-cx)**2+(y-cy)**2)*1.21))continue;if(!out[p]){out[p]=255;filled++;}for(const q of [x?p-1:-1,x<w-1?p+1:-1,y?p-w:-1,y<h-1?p+w:-1])if(q>=0&&!seen[q]){seen[q]=1;queue.push(q);}}
 }return {mask:out,accepted:filled>0,confidence:points.filter(p=>p.label===1).length?Math.round(hits/(points.filter(p=>p.label===1).length*8)*100):0,filled};
}
