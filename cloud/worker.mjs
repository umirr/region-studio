...
export default {async fetch(request,env){
 ...
 if(digest!==env.INVITE_SHA256)return json({error:'Invalid invitation code'},401);
 ...
 await env.SUBMISSIONS.put(key,request.body,{httpMetadata:{contentType:'application/zip'},onlyIf:{etagDoesNotMatch:'*'}});
 return json({submissionId:id,stored:true},201);
}};
