...
function registerCloudUpload({handle,app,atomic}){
 ...
 handle('beta-upload',async(id,code)=>{
  ...
  try{
   ...
   const data=await fs.readFile(receipt.path);if(data.readUInt32LE(0)!==0x04034b50)throw Error('ZIP 파일이 아닙니다.');
   const response=await fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/zip','X-Upload-Code':code,'X-Submission-Id':id},body:data,signal:AbortSignal.timeout(180000),redirect:'error'});
